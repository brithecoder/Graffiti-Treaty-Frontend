    ...
    const sketch = (p) => {
      ...
      p.draw = () => {
        ...
        if (strokes.length > 0 && currentIndexRef.current < strokes.length) {
          ...
          strokes.length > 0 &&
          currentIndexRef.current >= strokes.length
        ) {
          setIsFinished((prev) => {
            if (!prev) return true;
            return prev;
      ...
      const drawStroke = (stroke) => {
        ...
        const getX = (val) =>
          val <= 1 ? val * p.width : p.map(val, 0, 1200, 0, p.width);
        const getY = (val) =>
          val <= 1 ? val * p.height : p.map(val, 0, 675, 0, p.height);
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950 z-[200] flex flex-col items-center justify-center p-4">
      ...
      <div className="text-center mb-6">
        ...
        <div className="h-6">
          <p className="text-zinc-400 font-mono text-xs uppercase tracking-[0.3em]">
            ...
            <span>
              Wall: <span className="text-yellow-400/80">{wallCode}</span>
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-5xl aspect-video">
        ...
        <div
          ref={revealRef}
          className="w-full h-full border-8 border-white/5 shadow-2xl bg-black"
        />
      </div>
      {/* ACTION BUTTONS */}
      {isFinished && (
        <div className="mt-8 flex gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <button
            ...
            className="px-6 py-3 bg-[#ff007f] text-white font-bold uppercase tracking-tighter hover:bg-zinc-700 transition-all flex items-center gap-2"
          >
            ↺ Replay
          </button>
          <button
            onClick={handleDownload}
            className="px-8 py-3 bg-treaty-neon text-black font-black uppercase italic hover:scale-105 transition-all shadow-lg"
          >
            Download Piece
          </button>
          <button
            onClick={handleFinalExit}
            className="px-6 py-3 bg-white text-black font-bold uppercase tracking-tighter hover:bg-red-500 hover:text-white transition-all"
          >
            Finish Mission
          </button>
        </div>
