...
//           // 1. Create the new buffer FIRST at the new size
//           let newPg = p.createGraphics(newW, newH);

//           // 2. SCALE the old mural to fit the new buffer
//           // Syntax: image(source, x, y, targetWidth, targetHeight)
//           newPg.image(pg, 0, 0, newW, newH);

//           // 3. Resize the actual visible canvas
//           p.resizeCanvas(newW, newH);

//           // 4. Safely delete the old buffer from memory to prevent lag
//           pg.remove();

//           // 5. Swap the reference
//           pg = newPg;

//           // 6. Refresh the background
//           drawWall();
//         }
//       };
...
//         if (p.mouseIsPressed) {
//           // 🟢 RECORDER ADDITION: Save the path coordinate
//           if (currentStrokeRef.current) {
//             currentStrokeRef.current.points.push({ x: p.mouseX, y: p.mouseY });
//           }
...
//       p.mousePressed = () => {
//         // If the mouse is over the canvas (and not clicking a UI button)
//         // Only start recording if the mouse is actually inside the canvas area
//         // 🟢 Fix: Determine the tool based on your eraser state
//         if (
//           p.mouseX >= 0 &&
//           p.mouseX <= p.width &&
//           p.mouseY >= 0 &&
//           p.mouseY <= p.height
//         ) {
//           const currentTool = eraserRef.current
//             ? "eraser"
//             : `spray-${capRef.current}`;
//           currentStrokeRef.current = {
//             artist: artistName, // Use the prop
//             wall: wallCode,     // Use the prop
//             points: [],
...
//           console.log("Mural Data Updated:", historyRef.current);
...
//               <span className="text-[10px] uppercase text-gray-500 font-bold mb-1">
//                 Surface
//               </span>
